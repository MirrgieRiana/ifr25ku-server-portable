...
│       ├── neoforge-installer/                  NeoForge インストーラー JAR
│       └── modpacks/                            ダウンロード済み Modpack
│
├── .github/workflows/
...
| 関数 | 説明 |
|---|---|
| `getConfig()` | config.json を読み込んで返す (LAZY、ファイル不在時エラー) |
...
| `getDefaultModpackVersions()` | Modpack の全バージョンを semver-tool で昇順ソートして返す (キャッシュ付き) |
| `getVersionFromFile(filePath)` | ファイルから Modrinth バージョン情報を取得 (キャッシュ付き) |
| `install(targetDir, modpackVersion)` | NeoForge サーバーと Mod をインストール (前提: `curl`, `java`, `unzip`) |
...
| `build/` | Gradle ビルド生成物 |
| `.gradle/` | Gradle キャッシュ |
| `.ifr25ku-server-portable/` | 設定ファイル・ビルド生成物・キャッシュ (環境依存) |
| `/*tmp*/` | テスト用サーバーインスタンス |
